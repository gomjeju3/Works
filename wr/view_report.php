...
// 보고서 데이터 가져오기
$stmt = $conn->prepare("SELECT r.content, r.report_date, r.team_id, r.id, t.team_name FROM reports r JOIN teams t ON r.team_id = t.id WHERE r.id = :id");
...
// 지난 주 보고서 내용
$lastWeekStmt = $conn->prepare("SELECT content FROM reports WHERE team_id = :team_id AND report_date BETWEEN :start AND :end ORDER BY report_date ASC LIMIT 1");
