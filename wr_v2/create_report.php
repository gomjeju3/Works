...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($reportDate) || empty($teamId) || empty($userId) || empty($categoryId) || empty($content)) {
        echo "<script>alert('모든 필드를 채워주세요.'); history.back();</script>";
        exit;
    }

    $stmt = $conn->prepare("INSERT INTO reports (report_date, team_id, user_id, category_id, content) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$reportDate, $teamId, $userId, $categoryId, $content]);
